...
{"supplier":"string","issueDate":"YYYY-MM-DD or null","gross":number_or_null,"net":number_or_null,"vat":number_or_null,"currency":"3-letter ISO code","taxLines":[{"label":"string","rate":number,"netAmount":number_or_null,"taxAmount":number_or_null,"grossAmount":number_or_null}]}
...
{"supplier":"vendor company name","issueDate":"YYYY-MM-DD or null","gross":number_or_null,"net":number_or_null,"vat":number_or_null,"currency":"GBP/USD/EUR/AUD etc","taxLines":[{"label":"string","rate":number,"netAmount":number_or_null,"taxAmount":number_or_null,"grossAmount":number_or_null}]}
...
function findMoneyValues(text: string): number[] {
  ...
  return [...stripped.matchAll(/\b(\d{1,6}[.,]\d{2})\b/g)]
...
function parseSummaryTaxLinesFromText(rawText: string): AiTaxLine[] {
  ...
  for (const line of lines) {
    ...
    const bracketMatch = line.match(
      /VAT\s*\(?(\d+(?:[.,]\d+)?)%\)?[^0-9]*(\d{1,6}[.,]\d{2})$/i,
    );
    if (bracketMatch) {
      const rate = Number(bracketMatch[1].replace(",", "."));
      const taxAmount = Number(bracketMatch[2].replace(",", "."));
...
function buildDocument(
  ...
  const gross =
    fields.gross != null && fields.gross > 0
      ? fields.gross
      : grossFromLines;
  const net =
    fields.net != null && fields.net >= 0
      ? fields.net
      : netFromLines;
  const vat =
    fields.vat != null && fields.vat >= 0
      ? fields.vat
      : vatFromLines;
  ...
  return {
    ...
    fileName,
    supplier: fields.supplier?.trim() || fileName.replace(/\.[^.]+$/, ""),
...
function buildRegexOnlyDocument(
  ...
  const gross = values.length > 0 ? Math.max(...values) : undefined;
...
export async function extractDocumentFromBuffer(
  ...
  if (mimeType === "application/pdf") {
    ...
    const vision = await extractWithGeminiVision(buffer, mimeType);
    if (vision) {
...
export async function extractDocumentFromTextWithAI(
  ...
  return buildRegexOnlyDocument(fileName, mimeType, rawText, confidence);
}
